...
from data_provider.stock_data_factory import data_provider

from exp.exp_basic import Exp_Basic
from utils.tools import EarlyStopping, adjust_learning_rate, visual, save_to_csv, visual_weights, plot_loss_curve
...
class Stock_Forecast_DRIP(Exp_Basic):
    def __init__(self, args):
        super(Stock_Forecast_DRIP, self).__init__(args)
        self.clamp = 100
    ...
    def _build_model(self):
        ...
        if self.args.use_ckpt:
            model.load_state_dict(torch.load(self.args.ckpt_path), strict=False)
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_embed_id) in enumerate(vali_loader):
    ...
    def train(self, setting):
        train_data, train_loader = self._get_data(flag='train') # return data_set, data_loader
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_embed_id) in enumerate(train_loader):
                iter_count += 1
                model_optim.zero_grad()
                
                # batch_x, batch_y, _, _ = next(iter(train_loader))
                # print("batch_x:", torch.isnan(batch_x).any(), torch.isinf(batch_x).any(),
                #     batch_x.min(), batch_x.max())
                # print("batch_y:", torch.isnan(batch_y).any(), torch.isinf(batch_y).any(),
                ...
                if self.args.use_amp:
                    ...
                    #print('batch_y.shape', batch_y.shape) # 64, 1, 6
                    loss = criterion(outputs, batch_y)
                    
                    # # prevent loss explode
        ...
        plot_loss_curve(train_losses, test_losses, setting)
...
class Stock_Forecast(Exp_Basic):
    def __init__(self, args):
        super(Stock_Forecast, self).__init__(args)
        self.clamp = 3
    ...
    def train(self, setting):
        ...
        train_losses = []  # 每一個 epoch 結束時 append train_loss
        test_losses = []   # 每一個 epoch 結束時 append test_loss
